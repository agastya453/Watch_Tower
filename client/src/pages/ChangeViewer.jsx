...
export default function ChangeViewer() {
  ...
  const added = snapshot.diff_added
    ? JSON.parse(snapshot.diff_added)
    : [];

  const removed = snapshot.diff_removed
    ? JSON.parse(snapshot.diff_removed)
    : [];
  ...
  return (
    <div className="w-full">
      <Link
        to={`/history/${siteId}`}
        className="flex items-center text-sm font-medium text-gray-500 hover:text-apple-dark mb-8 gap-2 w-max"
      >
        <ArrowLeft size={16} />
        Back to Timeline
      </Link>
          ...
          <div className="flex-1 bg-white/50 rounded-2xl p-4 overflow-y-auto font-mono text-sm leading-relaxed border border-red-100 shadow-inner max-h-[500px]">
            {removed.length === 0 ? (
              ...
              removed.map((text, i) => (
                <div
                  key={i}
                  className="mb-2 bg-red-100 text-red-800 px-2 py-1 rounded inline-block mr-2 break-all"
                >
                  <del>{text}</del>
                </div>
              ))
          ...
          <div className="flex-1 bg-white/50 rounded-2xl p-4 overflow-y-auto font-mono text-sm leading-relaxed border border-green-100 shadow-inner max-h-[500px]">
            {added.length === 0 ? (
              ...
              added.map((text, i) => (
                <div
                  key={i}
                  className="mb-2 bg-green-100 text-green-800 px-2 py-1 rounded inline-block mr-2 break-all"
                >
                  {text}
                </div>
              ))
